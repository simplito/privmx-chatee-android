...
class PagingList<Value : Any>(
    ...
    private fun doLoad(
        ...
        if (enableLog) {
            Log.d(
                TAG,
                "DoLoad on page: ${start} size: ${size} next: ${nextPageKey.key} previous: ${previousPageKey.key} version: ${version} action: $action"
            )
        }
        return AsyncCall {
            ...
            if (it is LoadResult.Success) {
                ...
                if (enableLog) {
                    Log.d(
                        TAG,
                        "OnLoad page: ${start} size: ${size} next: ${nextPageKey.key} nextPageKeyOnStart: ${nextPageKeyOnStart.key} previous: ${previousPageKey.key} version: ${version} callVersion: ${callVersion} action: $action"
                    )
                }
                when (action) {
                    Action.PREPEND -> {
                ...
                if (enableLog) {
                    Log.d(
                        TAG,
                        "${currentKey.intValue} List of keys: ${list.mapIndexed { index, item -> firstKey + index }}"
                    )
                }
                onSuccesLoad(action)
    ...
    private fun onSuccesLoad(action: Action) {
        ...
        if (enableLog) {
            Log.d(TAG, "Load test: onSuccess action: $action")
            Log.d(
                TAG,
                "Load test: onSuccess: nextPageKey: ${nextPageKey.key}// list.size=${list.size}"
            )
        }
    }
    ...
    private fun collector(action: Action) {
        if (config.maxSize > 0 && config.maxSize > config.pageSize && list.size > config.maxSize && currentKey.intValue > 0) {
            ...
            if (enableLog) {
                Log.d(
                    TAG,
                    "Delete:\nPrevious: ${previousPageKey.key}\nFirst: ${firstKey}\ncurrent: ${currentKey.intValue}\nNext: ${nextPageKey.key}\n"
                )
            }
