...
class MainActivity : BasicActivity() {
    ...
    override fun onPrivmxEndpointStart() {
        ...
        ChateeSession.serverConnection?.cloudData?.let { cloudData ->
            threadsPager.value = PagingList(
                ...
                if (privmxEndpointContainer?.privmxEndpoint == null) {
                ...
                privmxEndpointContainer?.privmxEndpoint!!.let { privmxEndpoint ->
                    ...
                    try {
                        val result = privmxEndpoint.threadApi.threadList(
                            cloudData.contextId,
                            start.toLong(),
                            size.toLong(),
                            SortOrder.DESC
            ...
            privmxEndpointContainer.privmxEndpoint?.let { privmxEndpoint ->
                privmxEndpoint.registerCallback(
                    this@MainActivity,
                    EventType.UpdateThreadStatsEvent
                ) { threadStatsData ->
    ...
    private fun removeChat(thread: ThreadInfo): Promise<*> {
        return AsyncCall {
            privmxEndpointContainer?.privmxEndpoint?.let { privmxEndpoint ->
                val storeId = try {
                    ChateeJson.decodeFromString<ThreadNameJson>(thread.data?.title ?: "").storeId
                } catch (e: Exception) {
                    null
                }
                storeId?.let {
                    privmxEndpoint.storeApi?.storeDelete(it)
                    Log.d(TAG, "Store deleted")
                }
                privmxEndpoint.threadApi.threadDelete(thread.threadId)
                                ...
                                .filter {
                                    try {
                                        ChateeJson.decodeFromString<ThreadNameJson>(
                                            it.thread.data?.title ?: ""
                                        ).name.lowercase().contains(searchingValue.lowercase())
                                    } catch (e: Exception) {
                                        false
                                    }
                                }.toTypedArray()
                    ...
                    BottomSheet(
                        ...
                        threadToDelete?.let {
                            if (currentRequest == null) {
                                currentRequest = removeChat(it.thread)
                                    .then {
                                        coroutineScope.launch {
                                            threadToDelete = null
                                            currentRequest = null
...
private fun ThreadInfo.fromNewStats(threadStatsData: ThreadStatsEventData) = ThreadInfo(
    threadId,
    contextId,
    createDate,
    creator,
